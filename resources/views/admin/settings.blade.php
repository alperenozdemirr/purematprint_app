        ...
        <div>
          @if ($setting->hasCustomIntroImage())
            <div class="mb-4 flex items-center gap-4">
              <img src="{{ $setting->introImageUrl() }}" alt="Masaüstü giriş görseli" class="h-24 w-auto max-w-[280px] rounded-lg border border-ink/10 bg-cream object-cover p-1">
              <p class="font-body text-[13px] text-muted">Masaüstü görseli yüklü.</p>
            </div>
          @else
            <p class="mb-4 font-body text-[13px] text-muted">Masaüstü görseli yok — varsayılan kullanılır.</p>
          @endif
          <label for="intro_image" class="mb-1.5 block font-body text-[13px] font-bold text-ink">Giriş Görseli — Masaüstü</label>
          <input type="file" id="intro_image" name="intro_image" accept="{{ \App\Support\ImageUploadRules::acceptAttribute() }}"
                 class="w-full rounded-lg border border-ink/10 bg-cream px-3.5 py-2.5 font-body text-[14px] text-ink outline-none focus:border-accent focus:ring-2 focus:ring-accent/15">
          <p class="mt-1.5 font-body text-[12px] text-muted">{{ \App\Support\ImageUploadRules::humanList() }} — en fazla 100MB. Önerilen oran: 16:9 veya 2:1.</p>
          @error('intro_image') <p class="mt-1.5 font-body text-[12px] font-medium text-danger">{{ $message }}</p> @enderror
        </div>
        <div>
          @if ($setting->hasCustomIntroImageMobile())
            <div class="mb-4 flex items-center gap-4">
              <img src="{{ $setting->introImageMobileUrl() }}" alt="Mobil giriş görseli" class="h-24 w-auto max-w-[280px] rounded-lg border border-ink/10 bg-cream object-cover p-1">
              <p class="font-body text-[13px] text-muted">Mobil görseli yüklü.</p>
            </div>
          @else
            <p class="mb-4 font-body text-[13px] text-muted">Mobil görseli yok — masaüstü (veya varsayılan) kullanılır.</p>
          @endif
          <label for="intro_image_mobile" class="mb-1.5 block font-body text-[13px] font-bold text-ink">Giriş Görseli — Mobil</label>
          <input type="file" id="intro_image_mobile" name="intro_image_mobile" accept="{{ \App\Support\ImageUploadRules::acceptAttribute() }}"
                 class="w-full rounded-lg border border-ink/10 bg-cream px-3.5 py-2.5 font-body text-[14px] text-ink outline-none focus:border-accent focus:ring-2 focus:ring-accent/15">
          <p class="mt-1.5 font-body text-[12px] text-muted">İsteğe bağlı. Boş bırakılırsa masaüstü görseli kullanılır. Önerilen oran: 4:5 veya 3:4.</p>
          @error('intro_image_mobile') <p class="mt-1.5 font-body text-[12px] font-medium text-danger">{{ $message }}</p> @enderror
        </div>
        <div class="md:col-span-2">
