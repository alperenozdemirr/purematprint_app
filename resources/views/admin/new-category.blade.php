...
@section('breadcrumb', 'Katalog / Kategoriler / Yeni')
...
@section('content')
  <div class="mb-6 flex items-center gap-3">
    ...
    <div>
      <h2 class="font-heading text-[22px] font-bold leading-tight text-ink">Yeni Kategori Ekle</h2>
      <p class="font-body text-[13px] text-muted">Kategori bilgilerini doldurup kaydedin</p>
    </div>
  </div>
    ...
    <div class="mx-auto max-w-2xl overflow-hidden rounded-xl bg-surface shadow-card">
      ...
      <div class="grid grid-cols-1 gap-5 p-5">
        ...
        <div>
          ...
          <p class="rounded-lg border border-ink/10 bg-cream/60 px-3.5 py-2.5 font-body text-[14px] text-muted">Kategori adı + 6-9 haneli benzersiz sayı ile otomatik oluşturulur</p>
        </div>
        ...
        <div>
          ...
          <select id="parent_id" name="parent_id" class="w-full rounded-lg border border-ink/10 bg-cream px-3.5 py-2.5 font-body text-[14px] font-medium text-ink outline-none transition-colors focus:border-accent focus:ring-2 focus:ring-accent/15">
            ...
            @foreach ($parentOptions as $parentOption)
              <option value="{{ $parentOption['id'] }}" @selected(old('parent_id') == $parentOption['id'])>{{ $parentOption['label'] }}</option>
            @endforeach
          </select>
          <p class="mt-1.5 font-body text-[12px] text-muted">Herhangi bir kategorinin altına alt kategori ekleyebilirsiniz.</p>
