            ...
            <div>
              ...
              <div class="grid grid-cols-2 gap-4 sm:grid-cols-4">
                <div>
                  ...
                  <input type="number" step="0.001" min="0.001" id="shipping_weight" name="shipping_weight" value="{{ old('shipping_weight', $product->shipping_weight) }}"
                ...
                <div>
                  ...
                  <input type="number" min="1" id="shipping_length" name="shipping_length" value="{{ old('shipping_length', $product->shipping_length) }}"
                         class="w-full rounded-lg border border-ink/10 bg-cream px-3 py-2.5 font-body text-[14px] text-ink outline-none focus:border-accent"
                         placeholder="20">
                ...
                <div>
                  ...
                  <input type="number" min="1" id="shipping_width" name="shipping_width" value="{{ old('shipping_width', $product->shipping_width) }}"
                         class="w-full rounded-lg border border-ink/10 bg-cream px-3 py-2.5 font-body text-[14px] text-ink outline-none focus:border-accent"
                         placeholder="15">
                ...
                <div>
                  ...
                  <input type="number" min="1" id="shipping_height" name="shipping_height" value="{{ old('shipping_height', $product->shipping_height) }}"
                         class="w-full rounded-lg border border-ink/10 bg-cream px-3 py-2.5 font-body text-[14px] text-ink outline-none focus:border-accent"
                         placeholder="10">
        ...
        <section class="overflow-hidden rounded-xl bg-surface shadow-card">
          ...
          <div class="p-5">
            @if ($product->images->isNotEmpty())
              <div class="mb-5 grid grid-cols-3 gap-3 sm:grid-cols-4" data-existing-images>
                @foreach ($product->images as $image)
                  <div class="group relative aspect-square cursor-grab overflow-hidden rounded-lg bg-cream active:cursor-grabbing" data-image-id="{{ $image->id }}">
                    <input type="hidden" name="existing_image_order[]" value="{{ $image->id }}">
                    <img src="{{ $image->url }}" alt="" class="h-full w-full object-cover pointer-events-none">
                    <span class="absolute left-1 top-1 rounded bg-ink/70 px-1.5 py-0.5 font-body text-[10px] font-bold uppercase text-on-dark {{ $loop->first ? '' : 'hidden' }}" data-cover-badge>Kapak</span>
...
@section('scripts')
  ...
  <script>
    (function () {
      ...
      const updateExistingCoverBadges = () => {
        if (!existing) return;
        existing.querySelectorAll('[data-cover-badge]').forEach((badge, index) => {
      ...
      const renderPreview = () => {
        ...
        selectedFiles.forEach((file) => {
          ...
          cell.innerHTML = '<img src="' + URL.createObjectURL(file) + '" class="pointer-events-none h-full w-full object-cover" alt="">' +
            '<span class="absolute left-1 top-1 rounded bg-ink/70 px-1.5 py-0.5 font-body text-[10px] font-bold uppercase text-on-dark">Yeni</span>';
          preview.appendChild(cell);
        });
      };

      const ensurePreviewSortable = () => {
      ...
      if (existing && typeof Sortable !== 'undefined') {
        Sortable.create(existing, {
          animation: 150,
          onEnd: () => updateExistingCoverBadges(),
        });
      }
