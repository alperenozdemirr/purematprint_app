...
@section('page_title', 'Yeni Banner')
@section('breadcrumb', 'İçerik / Bannerlar / Yeni')

@section('content')
  <div class="mb-6 flex items-center gap-3">
    <a href="{{ route('admin.bannerList') }}" aria-label="Geri"
    ...
    <div>
      <h2 class="font-heading text-[22px] font-bold leading-tight text-ink">Yeni Banner Ekle</h2>
      <p class="font-body text-[13px] text-muted">Banner bilgilerini ve görselini yükleyin</p>
    </div>
  </div>
  ...
  <form action="{{ route('admin.bannerStore') }}" method="POST" enctype="multipart/form-data">
    ...
    <div class="grid grid-cols-1 gap-6 xl:grid-cols-[1fr_360px]">
        ...
        <section class="overflow-hidden rounded-xl bg-surface shadow-card">
          ...
          <div class="grid grid-cols-1 gap-5 p-5">
            <div>
              ...
              <input type="text" id="sub_title" name="sub_title" value="{{ old('sub_title') }}" required
            ...
            <div>
              ...
              <input type="text" id="title" name="title" value="{{ old('title') }}" required
                     class="w-full rounded-lg border border-ink/10 bg-cream px-3.5 py-2.5 font-body text-[14px] text-ink outline-none transition-colors focus:border-accent focus:ring-2 focus:ring-accent/15"
                     placeholder="Örn. LED Tabela Kampanyası">
            ...
            <div>
              ...
              <textarea id="description" name="description" rows="3" required
                        ...
                        placeholder="Banner açıklaması...">{{ old('description') }}</textarea>
            ...
            <div>
              ...
              <input type="number" min="0" id="number" name="number" value="{{ old('number', 0) }}"
                     class="w-full rounded-lg border border-ink/10 bg-cream px-3.5 py-2.5 font-body text-[14px] text-ink outline-none transition-colors focus:border-accent focus:ring-2 focus:ring-accent/15">
      ...
      <aside class="flex flex-col gap-6">
        <section class="overflow-hidden rounded-xl bg-surface shadow-card">
          <div class="border-b border-ink/10 px-5 py-4">
            <h3 class="font-heading text-[16px] font-bold text-ink">Banner Görseli</h3>
          </div>
          <div class="p-5">
        ...
        <div class="flex flex-col gap-3">
          ...
          <a href="{{ route('admin.bannerList') }}" class="inline-flex w-full items-center justify-center rounded-lg bg-cream px-5 py-3 font-body text-[13px] font-bold uppercase tracking-[0.06em] text-ink transition-colors hover:bg-hover">
            İptal
          </a>
        </div>
      </aside>
