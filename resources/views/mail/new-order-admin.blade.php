        ...
        <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="max-width:560px;background:{{ $mailSurface }};border:3px solid {{ $mailInk }};">
          ...
          <tr>
            <td style="padding:28px 24px;border-bottom:3px solid {{ $mailInk }};background:{{ $mailAnnounce }};color:{{ $mailOnDark }};">
              ...
              <h1 style="margin:10px 0 0;font-size:26px;line-height:1.2;font-family:{{ $mailFontHeading }}">Yeni Sipariş Aldınız</h1>
            </td>
          </tr>
          <tr>
            <td style="padding:28px 24px;">
              ...
              @if ($order->note)
              <p style="margin:0 0 8px;font-size:12px;font-weight:700;letter-spacing:0.08em;text-transform:uppercase;color:{{ $mailAnnounce }};">Müşteri Notu</p>
              <p style="margin:0 0 20px;font-size:14px;line-height:1.7;color:{{ $mailMuted }};">{{ $order->note }}</p>
              @endif
